/**
 * 🔄 Examples Directory - Prescription Calculator Service
 * 原项目: B2B2C中医处方履约平台
 * 复用等级: 一级复用 (90% 复用价值)
 ...
 * 测试覆盖: 单元测试覆盖率>80%
 * 
 * @description 高价值可复用的财务计算引擎，适用于任何需要精确金额计算的场景
 * @usage 可直接复制到项目中，或作为参考实现类似计算逻辑
 * @integration 支持Edge Functions环境，可与Supabase完美集成
 * 
 * 🚨 使用方法：
 * ```typescript
 * import { PrescriptionCalculatorService } from './prescription-calculator.service'
 * 
 ...
 *   medicines: [
 *     { medicineId: 'med1', weight: 10.5, basePrice: 3.33 },
 *     { medicineId: 'med2', weight: 15.0, basePrice: 2.50 }
 *   ],
 *   copies: 7
 * })
 * ```
