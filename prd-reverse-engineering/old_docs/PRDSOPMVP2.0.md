...
#### 2. 手动审核与操作
**业务描述**：管理员手动审核PO和价目表，重点控制利润率  
**技术要求**：
- 手动审核API：`PATCH /api/v1/purchase-orders/:id/approve`
- 价目表审核：重点检查药品价差和利润率
- 利润率控制：确保15-30%平台利润空间
- 简单日志：基础操作记录
...
#### 4. 用户账户管理（MVP2.0闭环控制）
**业务描述**：管理员创建和维护医师、药房账户，关闭公开注册  
**技术要求**：
- 医师账户创建：`POST /api/v1/admin/practitioners`
- 药房账户创建：`POST /api/v1/admin/pharmacies`
- 账户状态管理：`PATCH /api/v1/admin/users/:id/status`
- 密码重置：`POST /api/v1/admin/users/:id/reset-password`
- 邮箱唯一性验证和执照验证机制
...
#### 5. 系统监控与日志
**业务描述**：API调用日志查询和系统运行状态监控  
**技术要求**：
- API日志查询：`GET /api/v1/admin/api-logs`
- 实时监控：`GET /api/v1/admin/api-logs/realtime`
- 日志统计分析：`GET /api/v1/admin/api-logs/statistics`
- 数据自动脱敏和定期归档机制
