...
function populateTemplate(template: string, data: EmailTemplateRequest): string {
  ...
  return template
    .replace(/\{\{\s*\.ConfirmationURL\s*\}\}/g, `${data.email_data.site_url}/auth/confirm?token_hash=${data.email_data.token_hash}&type=${data.email_data.email_action_type}&redirect_to=${data.email_data.redirect_to || data.email_data.site_url}`)
...
serve(async (req) => {
  try {
    ...
    const requestData: EmailTemplateRequest = await req.json()
