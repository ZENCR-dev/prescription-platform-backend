...
#### Layer3: 执行Atomic层 (Agent自主生成)
- **职责范围**: 4-8小时原子任务执行、TDD-Todos循环、代码实现
...
#### SuperClaude工具集成指引
- **任务标注要求**: 生成TASK0X.md时，每个最小单位任务必须标注推荐的SuperClaude命令(/sc:implement, /sc:build, /sc:analyze等)、角色(--persona-backend, --persona-security等)和MCP服务器(--seq, --c7, --magic, --play)
- **参考文档**: 根据`prd-reverse-engineering/drafts/Superclaudev3README (1).md`选择最适合的工具组合，确保开发效率最大化
...
### Git工作流规范
- **分支策略**: GitFlow简化版 (main/develop/feature/hotfix)
